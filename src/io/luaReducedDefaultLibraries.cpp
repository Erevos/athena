...
namespace athena
{
	...
	namespace io
	{
		...
		utility::Timer LuaReducedDefaultLibraries::s_timer;
		//	An array holding the name of each function and a pointer to the function.
		luaL_Reg LuaReducedDefaultLibraries::s_functions[26] = {
			{ "assert" , LuaReducedDefaultLibraries::assert } ,
			{ "error" , LuaReducedDefaultLibraries::error } ,
			{ "getmetatable" , LuaReducedDefaultLibraries::getmetatable } ,
			{ "ipairs" , LuaReducedDefaultLibraries::ipairs } ,
			{ "next" , LuaReducedDefaultLibraries::next } ,
			{ "pairs" , LuaReducedDefaultLibraries::pairs } ,
			{ "printerror" , LuaReducedDefaultLibraries::print_error } ,
			{ "printwarning" , LuaReducedDefaultLibraries::print_warning } ,
			{ "print" , LuaReducedDefaultLibraries::print } ,
			{ "rawequal" , LuaReducedDefaultLibraries::rawequal } ,
			{ "rawlen" , LuaReducedDefaultLibraries::rawlen } ,
			{ "rawget" , LuaReducedDefaultLibraries::rawget } ,
			{ "rawset" , LuaReducedDefaultLibraries::rawset } ,
			{ "select" , LuaReducedDefaultLibraries::select } ,
			{ "setmetatable" , LuaReducedDefaultLibraries::setmetatable } ,
			{ "tonumber" , LuaReducedDefaultLibraries::tonumber } ,
			{ "tostring" , LuaReducedDefaultLibraries::tostring } ,
			{ "toboolean" , LuaReducedDefaultLibraries::toboolean } ,
			{ "type" , LuaReducedDefaultLibraries::type } ,
			{ "clock" , LuaReducedDefaultLibraries::clock } ,
			{ "date" , LuaReducedDefaultLibraries::date } ,
			{ "difftime" , LuaReducedDefaultLibraries::difftime },
			{ "time" , LuaReducedDefaultLibraries::time } ,
			{ NULL , NULL }
		};
		...
		*/
		...
		int LuaReducedDefaultLibraries::assert( lua_State* state )
		{
		...
		int LuaReducedDefaultLibraries::error( lua_State* state )
		{
		...
		int LuaReducedDefaultLibraries::getmetatable( lua_State* state )
		{
		...
		int LuaReducedDefaultLibraries::next( lua_State* state  )
		{
			int return_value = 1;
		...
		int LuaReducedDefaultLibraries::pairs (lua_State *L)
		{
			return pairsmeta(L,"__pairs",0,LuaReducedDefaultLibraries::next);
		}
		...
		int LuaReducedDefaultLibraries::ipairs( lua_State* state  )
		{
			return pairsmeta(state, "__ipairs",1,ipairsaux);
		}

		int LuaReducedDefaultLibraries::print_error( lua_State* state )
		{
			std::string message("");
		...
		int LuaReducedDefaultLibraries::print_warning( lua_State* state )
		{
			std::string message("");
		...
		int LuaReducedDefaultLibraries::print( lua_State* state )
		{
			std::string message("");
		...
		int LuaReducedDefaultLibraries::rawequal (lua_State *L)
		{
			luaL_checkany(L,1);
			luaL_checkany(L,2);
		...
		int LuaReducedDefaultLibraries::rawlen( lua_State* state )
		{
		...
		int LuaReducedDefaultLibraries::rawget( lua_State* state )
		{
			luaL_checktype(state,1,LUA_TTABLE);
		...
		int LuaReducedDefaultLibraries::rawset( lua_State* state )
		{
			luaL_checktype(state,1,LUA_TTABLE);
			luaL_checkany(state,2);
		...
		int LuaReducedDefaultLibraries::select( lua_State* state )
		{
		...
		int LuaReducedDefaultLibraries::setmetatable( lua_State* state )
		{
		...
		int LuaReducedDefaultLibraries::tonumber( lua_State* L )
		{
			int return_value = 1;
			bool push_nil = true;
		...
		int LuaReducedDefaultLibraries::toboolean( lua_State* state )
		{
			if ( lua_gettop(state) > 0 ) 
			{
				...
				if ( !lua_isnil(state,1) )
				{
					if ( lua_isnumber(state,1) )
					{
						...
						LuaState::get_integer(state,1,number);
					...
					else if ( lua_isstring(state,1) )
					{
						...
						LuaState::get_string(state,1,text);
		...
		int LuaReducedDefaultLibraries::tostring( lua_State* state )
		{
			luaL_checkany(state,1);
		...
		int LuaReducedDefaultLibraries::type( lua_State* state )
		{
			luaL_checkany(state,1);
		...
		int LuaReducedDefaultLibraries::clock( lua_State* state )
		{
		...
		int LuaReducedDefaultLibraries::date( lua_State* state )
		{
		...
		int LuaReducedDefaultLibraries::difftime( lua_State* state )
		{
		...
		int LuaReducedDefaultLibraries::time( lua_State* state )
		{
			lua_Number value = -1;
			...
			if ( args > 0 )  /* called without args? */
			{
				...
				if ( LuaState::get_string(state,1,type) )
				{
					if ( type.compare(0,2,"ns",2) == 0 )
						value = s_timer.nanoseconds();
					else if ( type.compare(0,2,"mcs",2) == 0 )
						value = s_timer.microseconds();
					else if ( type.compare(0,2,"ms",2) == 0 )
						value = s_timer.milliseconds();
					else
						value = s_timer.seconds();
				}
				else
					value = s_timer.seconds();
			}
			else
				value = s_timer.seconds(); /* get current time in seconds */
		...
		LuaReducedDefaultLibraries::LuaReducedDefaultLibraries()
		{
		}
		...
		int LuaReducedDefaultLibraries::open_reducedlibraries( lua_State* state )
		{
			/* set global _G */
			...
			luaL_setfuncs(state,s_functions,0);
